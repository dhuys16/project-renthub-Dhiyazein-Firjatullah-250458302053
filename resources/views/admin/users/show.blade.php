    ...
    <div class="flex flex-wrap mb-6">
        <div class="flex-none w-full px-3">
            <div class="flex justify-between items-center bg-white p-6 rounded-xl shadow-xl">
                <h6 class="dark:text-white text-xl font-bold">Detail Pengguna: {{ $user->username }}</h6>
        ...
        <div class="w-full max-w-full px-3 lg:w-4/12">
            <div class="relative flex flex-col min-w-0 break-words bg-white border-0 border-solid shadow-xl rounded-2xl bg-clip-border text-center">
                <div class="p-6">
                    ...
                    <img src="{{ $photoPath }}" 
                         alt="Foto Profil"
                         class="inline-flex items-center justify-center mx-auto mb-4 h-24 w-24 rounded-full object-cover border-4 border-{{ $user->role === 'vendor' ? 'indigo' : 'green' }}-300">
                    
                    <h5 class="font-bold text-lg text-slate-700">{{ $user->name }}</h5>
                    ...
                    <div class="mt-4 p-3 bg-gray-50 rounded-lg">
                        <p class="text-sm font-semibold text-slate-600">Total Produk: {{ $user->products_count ?? '0' }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="relative flex flex-col min-w-0 break-words bg-white border-0 border-solid shadow-xl rounded-2xl bg-clip-border">
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm text-slate-600">
                        ...
                        <div>
                            <p class="font-semibold text-slate-700">NIK:</p>
                            <p class="mb-3">{{ $user->nik ?? '-' }}</p>
                        </div>
                        
                        <div>
                            <p class="font-semibold text-slate-700">Terdaftar Sejak:</p>
                            <p class="mb-3">{{ $user->created_at->format('d M Y') }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mb-3">
                                ...
                                <span class="text-xs font-semibold {{ $user->is_verified ? 'text-green-500' : 'text-red-500' }}">
                    ...
                    <div class="text-sm text-slate-600">
                        ...
                        <p class="mb-3 whitespace-pre-wrap">{{ $user->address ?? 'N/A' }}</p>
                        
                        <p class="font-semibold text-slate-700">Link Google Maps:</p>
                        <p>
                            @if ($user->link_gmaps)
                                <a href="{{ $user->link_gmaps }}" target="_blank" class="text-blue-500 hover:text-blue-700 underline">
                                    Lihat Lokasi
                                </a>
                            @else
                                N/A
